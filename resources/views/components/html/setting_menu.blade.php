<div class="d-flex">
    @foreach (Config::get('menu.setting') as $setting)
        <div class="card bg-highlight mt-5 mb-3 d-inline-flex mx-2 setting-link {{ (Config::get('app.url').'/'.Request::path()) == route($setting['route_name']) ? 'text-main' : 'text-muted'}}" style="flex: 1;" data-url="{{ route($setting['route_name']) }}">
